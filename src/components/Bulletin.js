        ...
        <div className="Bulletin">
            {isEditing ? (
                <>
                    ...
                    />
                    <button onClick={handleSubmit} data-testid="save-button">Сақтау</button>
                </>
            ) : (
                <>
                    <h3>{bulletin.title}</h3>
                    <p>{bulletin.content}</p>
                    <button onClick={handleEdit} data-testid="edit-button">Өзгерту</button>
                    <button onClick={handleDelete} data-testid="delete-button">Өшіру</button>
                </>
            )}
        </div>
    );
